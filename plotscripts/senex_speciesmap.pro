; $Id: senex_speciesmap.pro,v 1.7 2008/07/06 17:07:19 jaf Exp $
;-----------------------------------------------------------------------
;+
; NAME:
;        SENEX_SPECIESMAP
;
; PURPOSE:
;        Plot a given species measured on the SENEX aircraft or modeled
;        by GEOS-Chem on a domain appropriate for the SENEX missions.
;
; CATEGORY:
;
; CALLING SEQUENCE:
;        SENEX_SPECIESMAP,Species,Platform[,Keywords]
;
; INPUTS:
...
pro senex_speciesmap, species, platform, flightdates=flightdates, $
...
if keyword_set(model) then begin
	print,'here!'
	data = get_model_data_senex(species,platform,flightdates, $
	                             _extra=_extra)
	lat = get_model_data_senex('lat',platform,flightdates, $
	                             _extra=_extra)
	lon = get_model_data_senex('lon',platform,flightdates, $
	                             _extra=_extra)
	alt_data = get_model_data_senex('alt',platform,flightdates, $
	                             _extra=_extra)
endif else begin
	data = get_field_data_senex(species,platform,flightdates,$
                                     _extra=_extra)
